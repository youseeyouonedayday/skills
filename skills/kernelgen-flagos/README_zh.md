...
KernelGen `kernelgen-flagos` 技能（Skills）是统一 AI 编程技能。通过调用 `kernelgen-mcp` KernelGen 算子开发 MCP 工具集服务，该技能兼容国内外多种芯片平台，可生成高精度 GPU 算子。它能够自动检测目标代码仓库的类型，并分发至对应的专属工作流执行。
